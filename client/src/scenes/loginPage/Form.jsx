...
const Form = () => {
    ...
    const isNonMobile = useMediaQuery("(mi-width: 1000px)");
    const isLogin = pageType === "login";
    const isRegister = pageType === "register";

    const register = async(values, onSubmitProps) => {
        // Using form data due to picture image
        ...
        <Formik onSubmit={handleFormSubmit} initialValues={isLogin ? initialValuesLogin: initialValuesRegister} validationSchema={isLogin ? loginSchema : registerSchema}>
            {({
                ...
                <form onSubmit={handleSubmit}>
                                    ...
                                    <Dropzone acceptedFiles=".jpg,.jpeg,.png" multiple={false} onDrop={(acceptedFiles) => {
                                        ...
                                        {({getRootProps, getInputProps}) => (
                                            <Box {...getRootProps()} border={`2px dashed ${palette.primary.main}`} p="1rem" sx={{"&:hover": {cursor: "pointer"}}}>
                                                <input {...getInputProps()} />
                                                {!values.picture ? (<p>Add Profile Picture Here</p>) : (<FlexBetween>
                    ...
                    {/** BUTTONS */}
                    <Box>
                        <Button fullWidth type="submit" sx={{m: "2rem 0", p: "1rem", background: palette.primary.main, color: palette.background.alt,
